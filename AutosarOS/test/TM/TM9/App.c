 ...
 * @file
 *
 * @brief       Application code for task management test procedure 9
 *
 * @date        2020-07-13
...
{
    TaskStateType state = SUSPENDED;
    StatusType stat = GetTaskState(Task1, &state);
...
TASK(Task3)
{
    ...
    if (first == true) {
        ...
        StatusType stat = ChainTask(Task3);
        assert(stat == E_OK);
    } else {
