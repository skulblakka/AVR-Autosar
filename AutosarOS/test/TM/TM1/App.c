...
TASK(Task1)
{
    StatusType stat = ActivateTask(INVALID_TASK);
    assert(stat == E_OS_ID);

    TaskStateType state = SUSPENDED;
    stat = GetTaskState(INVALID_TASK, &state);
    assert(stat == E_OS_ID);

    stat = ChainTask(INVALID_TASK);
    assert(stat == E_OS_ID);

    stat = ActivateTask(Task2);
    assert(stat == E_OK);

    stat = Schedule();
    assert(stat == E_OK);

    stat = GetResource(RES_SCHEDULER);
    assert(stat == E_OK);
    ...
    stat = ReleaseResource(RES_SCHEDULER);
    assert(stat == E_OK);
...
TASK(Task2)
{
    ...
    assert(stat == E_OS_LIMIT);

    stat = ActivateTask(Task2);
    assert(stat == E_OS_LIMIT);

    stat = ChainTask(Task1);
    assert(stat == E_OS_LIMIT);
