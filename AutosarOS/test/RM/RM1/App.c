 ...
 * @file
 *
 * @brief       Application code for resource management test procedure 1
 *
 * @date        2020-07-16
...
TASK(Task1)
{
    StatusType stat = GetResource(ResA);
    assert(stat == E_OS_ACCESS);

    stat = GetResource(INVALID_RESOURCE);
    assert(stat == E_OS_ID);
    ...
    stat = GetResource(Res5);
    assert(stat == E_OK);

    stat = GetResource(Res6);
    assert(stat == E_OK);

    stat = ReleaseResource(Res6);
    assert(stat == E_OK);

    stat = ReleaseResource(Res5);
    assert(stat == E_OK);

    stat = ReleaseResource(Res4);
    assert(stat == E_OK);
    ...
    stat = ReleaseResource(INVALID_RESOURCE);
    assert(stat == E_OS_ID);
