 ...
 * @file
 *
 * @brief       Application code for event management test procedure 3
 *
 * @date        2020-07-16
...
TASK(Task1)
{
    ...
    StatusType stat = GetTaskState(Task2, &state);
    assert(stat == E_OK && state == WAITING);

    stat = GetEvent(Task2, &mask);
    assert(stat == E_OK && mask == 0b0);
    ...
    stat = GetEvent(Task2, &mask);
    assert(stat == E_OK && mask == 0b11);

    stat = SetEvent(Task2, 0b100);
    assert(stat == E_OK);

    stat = GetEvent(Task2, &mask);
    assert(stat == E_OK && mask == 0b111);
