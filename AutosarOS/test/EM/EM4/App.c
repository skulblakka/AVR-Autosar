 ...
 * @file
 *
 * @brief       Application code for event management test procedure 4
 *
 * @date        2020-07-16
...
TASK(Task1)
{
    ...
    StatusType stat = SetEvent(Task2, 0b10);
    assert(stat == E_OK);

    stat = GetTaskState(Task2, &state);
    assert(stat == E_OK && state == WAITING);

    stat = GetEvent(Task2, &mask);
    assert(stat == E_OK && mask == 0b10);

    stat = ActivateTask(Task3);
    assert(stat == E_OK);

    stat = GetTaskState(Task3, &state);
    assert(stat == E_OK && state == READY);

    stat = SetEvent(Task3, 0b100);
    assert(stat == E_OK);

    stat = GetEvent(Task3, &mask);
    assert(stat == E_OK && mask == 0b100);
...
TASK(Task4)
{
    StatusType stat = SetEvent(Task2, 0b1);
    assert(stat == E_OK);
