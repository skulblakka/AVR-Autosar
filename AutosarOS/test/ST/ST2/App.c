 ...
 * @file
 *
 * @brief       Application code for schedule tables test procedure 2
 *
 * @date        2020-07-20
...
TASK(Task1)
{
    StatusType stat = StartScheduleTableAbs(ST1, 1);
    assert(stat == E_OK);
    ...
    stat = NextScheduleTable(ST1, ST2);
    assert(stat == E_OK);

    stat = GetScheduleTableStatus(ST2, &status);
    assert(stat == E_OK && status == SCHEDULETABLE_NEXT);
...
TASK(Task2)
{
    StatusType stat = WaitEvent(0b01);
    assert(stat == E_OK);

    stat = ClearEvent(0b01);
    assert(stat == E_OK);
...
TASK(Task3)
{
    StatusType stat = WaitEvent(0b10);
    assert(stat == E_OK);

    stat = ClearEvent(0b10);
    assert(stat == E_OK);
