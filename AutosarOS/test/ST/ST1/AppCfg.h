 ...
 * @file
 *
 * @brief       Application configuration for schedule tables test procedure 1
 *
 * @date        2020-07-20
...
OS_CONFIG_TASK_BEGIN
    ...
    OS_CONFIG_TASK_DEF(Task1, 4,  256,     1, AUTOSTART,      BASIC,      PREEMPTIVE,     NULL,    0b00)
OS_CONFIG_TASK_END
...
OS_CONFIG_SCHEDULE_TABLE_BEGIN
    /* Schedule table ST1 */
    OS_CONFIG_SCHEDULE_TABLE_DEF_BEGIN(ST1, Counter1, false, false, 3)
        OS_CONFIG_SCHEDULE_TABLE_EXPIRY_POINTS_BEGIN(1)
            OS_CONFIG_SCHEDULE_TABLE_EXPIRY_POINT_DEF_BEGIN( 0, 1, 0)
                OS_CONFIG_SCHEDULE_TABLE_TASK_ACTIONS_BEGIN
                    OS_CONFIG_SCHEDULE_TABLE_TASK_ACTION_DEF(Task1)
