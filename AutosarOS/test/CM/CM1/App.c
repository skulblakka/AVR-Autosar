 ...
 * @file
 *
 * @brief       Application code for counter management test procedure 1
 *
 * @date        2020-07-20
...
TASK(Task1)
{
    TickType value = 0;
    TickType tick = 0;
    StatusType stat = GetCounterValue(INVALID_COUNTER, &value);
    assert(stat == E_OS_ID);

    stat = GetCounterValue(Counter1, NULL);
    assert(stat == E_OS_PARAM_POINTER);

    stat = GetCounterValue(Counter1, &value);
    assert(stat == E_OK && value == 0);

    stat = IncrementCounter(INVALID_COUNTER);
    assert(stat == E_OS_ID);

    stat = IncrementCounter(SYSTEM_COUNTER);
    assert(stat == E_OS_ID);

    stat = IncrementCounter(Counter1);
    assert(stat == E_OK);

    stat = GetElapsedValue(Counter1, NULL, &value);
    assert(stat == E_OS_PARAM_POINTER);

    stat = GetElapsedValue(Counter1, &value, NULL);
    assert(stat == E_OS_PARAM_POINTER);

    stat = GetElapsedValue(INVALID_COUNTER, &value, &tick);
    assert(stat == E_OS_ID);

    value = Counter_Cfg[Counter1]->maxallowedvalue + 1;
    stat = GetElapsedValue(Counter1, &value, &tick);
    assert(stat == E_OS_VALUE);

    stat = GetCounterValue(Counter1, &value);
    assert(stat == E_OK && value == 1);

    stat = IncrementCounter(Counter1);
    assert(stat == E_OK);

    stat = GetElapsedValue(Counter1, &value, &tick);
    assert(stat == E_OK && value == 2 && tick == 1);

    stat = GetCounterValue(SYSTEM_COUNTER, &value);
    assert(stat == E_OK);

    stat = GetElapsedValue(SYSTEM_COUNTER, &value, &tick);
    assert(stat == E_OK);
